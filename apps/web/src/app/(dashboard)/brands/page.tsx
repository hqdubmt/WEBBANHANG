...
export default function BrandsPage() {
  ...
  const load = () => {
    setLoading(true);
    brandsApi.list().then(setItems).catch(() => setItems([])).finally(() => setLoading(false));
  };
  ...
  const remove = async (id: string) => {
    ...
    await brandsApi.delete(id).catch(() => {});
    load();
  };
  ...
  return (
    <div>
      ...
      <DataTable
        ...
        data={items}
        columns={[
          { key: 'name', label: 'Tên thương hiệu', render: (r) => <span className="font-medium text-gray-900">{r.name}</span> },
          { key: 'slug', label: 'Slug', render: (r) => <span className="font-mono text-xs text-gray-400">{r.slug}</span> },
          { key: 'country', label: 'Quốc gia', render: (r) => r.country || '-' },
          { key: 'website', label: 'Website', render: (r) => r.website ? <a href={r.website} target="_blank" className="text-indigo-500 hover:underline text-xs">{r.website}</a> : '-' },
      ...
      <Modal open={modal} onClose={() => setModal(false)} title="Thêm thương hiệu" size="sm">
        <div className="space-y-3">
          {[
            { key: 'name', label: 'Tên thương hiệu', placeholder: 'VD: Apple' },
            { key: 'description', label: 'Mô tả', placeholder: '' },
            { key: 'website', label: 'Website', placeholder: 'https://' },
            { key: 'country', label: 'Quốc gia', placeholder: 'VD: Mỹ' },
          ].map((f) => (
            <div key={f.key}>
