...
const TARGET_GROUPS_FILE = '/app/fb_data/fb_target_groups_mypham.json';
const GROUPS_META_FILE = '/app/fb_data/fb_groups_meta_mypham.json';

const SESSION_PATH = '/app/fb_data/fb_session_mypham.json';

@Injectable()
export class FacebookGroupsMyPhamService implements OnModuleInit {
  private readonly logger = new Logger(FacebookGroupsMyPhamService.name);
  ...
  private readonly PROFILE_DIR = '/app/fb_data/browser_profile_mypham';
  ...
  async postToGroups(
    ...
    const pageName = opts?.pageName || process.env.FACEBOOK_MYPHAM_PAGE_NAME || 'Chuyên Sale Mỹ Phẩm';
    const pageUrl  = opts?.pageUrl  || process.env.FACEBOOK_MYPHAM_PAGE_URL  || `https://www.facebook.com/${process.env.FACEBOOK_MYPHAM_PAGE_ID || ''}`;
    ...
    for (const p of products) {
      ...
      for (const groupUrl of groupUrls) {
        ...
        try {
          ...
          // process.env.FACEBOOK_MYPHAM_PAGE_ID (fanpage chính) làm target, nên khi đang đăng bằng Page
  ...
  async inviteGroupReactors(groupPostUrls: string[]): Promise<{ invited: number }> {
    const pageId = process.env.FACEBOOK_MYPHAM_PAGE_ID;
    const pageToken = process.env.FACEBOOK_MYPHAM_ACCESS_TOKEN;
    let invited = 0;
